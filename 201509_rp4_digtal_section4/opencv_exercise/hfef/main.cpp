...
void lpfilter(int type,CvMat *matD,CvMat *matH,float D0,float k)
{
	if (D0 < 0)
	{
	...
	case GLPF:
		{
			for (int u = 0; u < w; ++u)
				for (int v =0 ;v < h; ++v)
				{
                    ...
                    h=k*(1-h);
					*( (float *)CV_MAT_ELEM_PTR(*matH,u,v) ) = 1+h;
		...
		case BLPF:
			{
				for (int u = 0; u < w; ++u)
					for (int v =0 ;v < h; ++v)
					{
						...
						t = pow(Elem_D,2*n);
						*( (float *)CV_MAT_ELEM_PTR(*matH,u,v) ) = 1-1/(1+t);
					}
			}
			break;
		default:
...
int main()
{
	...
	int n=2;
	 float k=0.5;
	lpfilter(GLPF,matD,matH,D0,k);//巴特沃夫要加上n,高斯和理想滤波去掉n这一项;做高频强调或者提升时加上k
