	...
	case GLPF:
		{
			for (int u = 0; u < w; ++u)
				for (int v =0 ;v < h; ++v)
				{
                    ...
                    int rh=2,rl=0.25,c=1;
					h = exp(-c*(Elem_D*Elem_D)/(D0*D0));
                    h=(rh-rl)*(1-h)+rl;
...
int main()
{
    IplImage *src =  cvLoadImage("horgn.png",CV_LOAD_IMAGE_GRAYSCALE);
	...
	float D0 = 100.0;
	int n=2;
	lpfilter(GLPF,matD,matH,D0);//巴特沃夫要加上n,高斯和理想滤波去掉n这一项
