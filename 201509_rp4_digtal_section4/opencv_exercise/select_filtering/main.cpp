#include <opencv/cv.h>//程序无法保存生成图像，似乎由错误
#include "math.h"
...
void lpfilter(int type,CvMat *matD,CvMat *matH,float D0,float w1)
{
	if (D0 < 0)
	{
	...
	case GLPF:
		{
			for (int u = 0; u < w; ++u)
				...
				{
					float Elem_D,h;
					Elem_D = CV_MAT_ELEM(*matD,float,u,v);//用来访问矩阵每个元素的宏，这个宏只对单通道矩阵有效

					h = exp(-((Elem_D*Elem_D)-D0*D0)/(Elem_D*w1)*((Elem_D*Elem_D)-D0*D0)/(Elem_D*w1));
...
void fft2(IplImage *src,CvMat *dst)
{
    ...
    dft_A = cvCreateMat( src->height, src->width, CV_32FC2 );
    ...
    cvGetSubRect( dft_A, &tmp, cvRect(0,0, src->width, src->height));//创建一定区域
	...
	cvCopy(dft_A , dst);//dft_A和dst他俩大小不一样，不能直接复制
...
int main()
{
    IplImage *src =  cvLoadImage("orgn.png",CV_LOAD_IMAGE_GRAYSCALE);
	...
	float D0 = 5.0,w1=140.0;
	lpfilter(GLPF,matD,matH,D0,w1);//巴特沃夫要加上n,高斯和理想滤波去掉n这一项
	...
	cvShiftDFT(srcshift,srcshift);//将源图像移至坐标中心
